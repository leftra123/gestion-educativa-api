                ...
                <div className={`${menuOpen ? 'flex' : 'hidden'} mt-4 lg:flex lg:items-center lg:mt-0 space-x-4 transition-height duration-300 ease-in-out`}>
                    {["establecimiento", "docente", "asistentes", "DAEM"].map(path => {
                        ...
                        return (
                            <NavLink
                                to={`/${path}`}
                                className={`text-lg text-gray-900 dark:text-white hover:text-gray-900 dark:hover:text-gray-400 transition-transform duration-200 ease-in-out transform hover:scale-105 ${path === "asistentes" || path === "DAEM" ? 'opacity-50 pointer-events-none' : ''} ${match ? 'text-indigo-700' : ''}`}
                                key={path}
                            >
